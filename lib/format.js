...
module.exports = {
    ...
    markDownToHtml: function(item, fields, subfields){
        ...
        return _.mapValues(item, (v, k, o) => {
            ...
            if(subfields.indexOf(k) != -1 &&  !_.isArray(o[k])){
                    if(o[k].info && o[k].info != "undefined"){
                        o[k].info = parser(o[k].info);
                    }        
                    if(o[k].text && o[k].text != "undefined"){
                        o[k].text = parser(o[k].text);
                    }        
                }

                if( _.isArray(o[k])){
                  o[k] = _.map(o[k], v => {
                        return _.mapValues(v, k => {
                            return parser(k);
                       });
                    });
                }
