...
describe('Date Util', function () {
  ...
  it('returns sorted and filtered calendar items', () => {
    ...
    expect(result[0]).to.be.an('object').to.include({
      title: "title 2"
    });
    expect(result[2]).to.be.an('object').to.include({
      title: "title 4"
    });
  ...
  it('returns sorted (reverse order) and filtered calendar items', () => {
    ...
    expect(reversed[0]).to.be.an('object').to.include({
      title: "title 4"
    });
    expect(reversed[2]).to.be.an('object').to.include({
      title: "title 2"
    });
